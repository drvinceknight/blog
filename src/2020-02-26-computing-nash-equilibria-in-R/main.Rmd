...
For the column player we need to consider the transpose of \(B\) 
(as all of the code we have written is from the perspective of the row player):
...
get_vertices <- function(A){
  ...
  labels <- t(apply(vertices, 1, function(vertex){get_labels(vertex, M, b)}))
  cbind(vertices, labels)
}
```

```{r}
row_vertices <- get_vertices(A)
col_vertices <- get_vertices(t(B))
```
...
```{r}
is_fully_labeled <- function(x, y, number_of_row_strategies, number_of_col_strategies){
   dimension <- number_of_row_strategies + number_of_col_strategies
   row_labels <- (x[-c(1:number_of_row_strategies)] %% dimension)
   col_labels <- (y[-c(1:number_of_col_strategies)] + number_of_row_strategies) %% dimension
...
```{r}
# TODO Use for loops and write a tibble
obtain_nash_equilibra <- function(A, B){
  ...
  for (i in 1:number_of_row_vertices){
    ...
    for (j in 1:number_of_col_vertices){
      ...
      if (is_fully_labeled(x=row_vertex_and_labels, y=column_vertex_and_labels, number_of_row_strategies = number_of_row_strategies, number_of_col_strategies = number_of_col_strategies)){
...
```{r}
rps <- rbind(c(2, 1, 3), c(3, 2, 1), c(1, 3, 2))
get_vertices(rps)
```
```{r}
rps <- rbind(c(2, 1, 3), c(3, 2, 1), c(1, 3, 2)) - 2
rps_row <- rps + 2
rps_col <- -rps + 2
